...
function init!()
    ...
    _statein    = zeros(Float64, NX+2*HS, NZ+2*HS, NUM_VARS)
      ...
      for i in 1-HS:NX+HS
        ...
        for kk in 1:NQPOINTS
          for ii in 1:NQPOINTS
            ...
            statein[i,k,ID_DENS] = statein[i,k,ID_DENS] + r                         * qweights[ii]*qweights[kk]
            statein[i,k,ID_UMOM] = statein[i,k,ID_UMOM] + (r+hr)*u                  * qweights[ii]*qweights[kk]
            statein[i,k,ID_WMOM] = statein[i,k,ID_WMOM] + (r+hr)*w                  * qweights[ii]*qweights[kk]
            statein[i,k,ID_RHOT] = statein[i,k,ID_RHOT] + ( (r+hr)*(t+ht) - hr*ht ) * qweights[ii]*qweights[kk]
          end
        end
...
#   zzt = 8000.0
    ...
    for k in 1-HS:NZ+HS
        ...
        statein[:,k,ID_SHUM] .= (qq0 * exp(-z/zq1)*exp(-(z/zq2)^2)) * hy_dens_cell[k]
        if zzt <= z
           state[:,k,ID_SHUM] .= qqt * hy_dens_cell[k]
           statein[:,k,ID_SHUM] .= qqt * hy_dens_cell[k]
        end
...
function thermal!(x::Float64, z::Float64)
    ...
    t = t + sample_ellipse_cosine!(x,z,1.0,XLEN/2,DZ,DX,DZ) 
